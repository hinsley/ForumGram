...
function BoardDestination({ scope, assertDestination }: { scope: AccountScope; assertDestination(): void }) {
	...
	return <div className={`content${isSidebarCollapsed ? ' sidebar-collapsed' : ''}`}>
		...
		<main className="main">
			...
			<header className="page-heading"><div className="row" style={{ alignItems: 'center', gap: 16 }}><div aria-hidden="true" style={{ width: 48, height: 48, borderRadius: 14, background: 'var(--border)', display: 'grid', placeItems: 'center', flexShrink: 0, overflow: 'hidden' }}>{forumAvatarUrl ? <img src={forumAvatarUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 20, color: 'var(--muted)', fontWeight: 700 }}>{forumTitle.charAt(0).toUpperCase()}</span>}</div><div style={{ minWidth: 0 }}><div className="eyebrow">{activeThreadId ? 'Thread' : 'Board'}</div><h1>{activeThreadId ? activeThread?.title ?? 'Thread' : boardTitle}</h1><p className="muted">{activeThreadId ? `A conversation in ${boardTitle}.` : boardMeta?.description || 'Browse the latest threads or start a conversation.'}</p></div></div></header>
			{error && <div className="alert" role="alert">{error}<button className="btn ghost" onClick={() => setError(null)}>Dismiss</button></div>}
			{submitted && <div className="alert row" role="status"><span>Post saved. Shown below while Telegram's search index catches up.</span><button className="btn ghost" onClick={() => setSubmitted(false)}>Dismiss</button></div>}
			{!activeThreadId ? <section className="card board-panel" aria-labelledby="threads-heading">
				...
				{threadQuery.isLoading ? <div className="empty-state" role="status">Loading threads in this board…</div> : threadQuery.error ? <div className="alert" role="alert"><strong>Could not load threads</strong><p>{threadQuery.error.message}</p></div> : !threads.length ? <div className="empty-state"><h3>No threads yet</h3><p>Start the first thread to get the conversation going.</p></div> : <div className="gallery boards">{threads.map(thread => <div key={thread.id} className="chiclet">
				...
				<div className="section-heading"><h2>Posts</h2><nav className="row" aria-label="Post pages" style={{ alignItems: 'center', gap: 6, flexWrap: 'wrap' }}><button className="btn ghost" disabled={canonicalPage <= 1} onClick={() => goPage(1)}>First</button><button className="btn" disabled={canonicalPage <= 1} onClick={() => goPage(canonicalPage - 1)}>Previous</button><span className="muted" aria-live="polite">Page {canonicalPage} of {totalPages} search windows</span><button className="btn" disabled={canonicalPage >= totalPages} onClick={() => goPage(canonicalPage + 1)}>Next</button><button className="btn ghost" disabled={canonicalPage >= totalPages} onClick={() => goPage(totalPages)}>Last</button></nav></div>
				<p className="muted">Live pages can shift as posts change. Search totals include invalid matches; a page may contain fewer posts.</p>
				{posts.isLoading && !messages.length ? <div className="empty-state" role="status">Loading posts in this thread…</div> : posts.error ? <div className="alert" role="alert"><strong>Could not load posts</strong><p>{posts.error.message}</p><button className="btn" onClick={() => void posts.refetch()}>Retry</button></div> : !messages.length ? <div className="empty-state"><h3>No posts yet</h3><p>Be the first to reply and start the conversation.</p></div> : <MessageList messages={messages.length ? messages : EMPTY_MESSAGES} currentUserId={me?.id} onEditPost={onEditPost} onDeletePost={onDeletePost} />}
