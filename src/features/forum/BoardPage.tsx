...
export default function BoardPage() {
	...
	async function onEditPost(m: any) {
		try {
			const newContent = prompt('Edit post content?', m.text)?.trim();
			if (newContent === undefined || newContent === null) return;
			if (!m.cardId || !m.threadId) { alert('Missing post metadata'); return; }
			const input = getInputPeerForForumId(forumId);
			const newText = composePostCard(m.cardId, String(m.threadId), { content: newContent });
			await editMessage(input, m.id, newText);
	...
	async function onDeletePost(m: any) {
		try {
			if (!confirm('Delete this post?')) return;
			const input = getInputPeerForForumId(forumId);
			await deleteMessages(input, [m.id]);
			await new Promise((r) => setTimeout(r, 250));
			await refetchPosts();
		} catch (e: any) {
