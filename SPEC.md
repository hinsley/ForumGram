...
**Target**: A fully client‑side forums‑only Telegram client as a **Vite + React + TypeScript PWA**, deployed to **Vercel** at **forum-gram.vercel.app**. No custom backend. All data lives in Telegram groups. Local cache/index lives in IndexedDB.
...
* **Goals**
  ...
  * Forums UX (phpBB/Discord‑Forums style) on top of Telegram.
  * Fast read/write, smooth navigation on huge forums.
  * Client‑side moderation, anti‑spam, and optional forum‑wide backups.
  * Markdown + KaTeX rendering, media upload/view.
  * Client‑side sub‑threads within a Board using signed tags for fast server‑side search.
  * Discovery of forums by `@handle`/invite and directory channels; local “catalogue”.
* **Non‑Goals (v1)**
...
* **Workers**:
  ...
  * Web Worker for text indexing & search (FlexSearch) per Board.
  * Service Worker (Workbox) for offline shell & media cache.
* **Telegram**: GramJS (MTProto) browser client; session persisted in IndexedDB.
* **Rendering**: `react-markdown` + `remark-gfm` + `rehype-katex` (KaTeX) + `rehype-highlight` + `rehype-sanitize`.
* **Virtualization**: `react-virtuoso` for board & message lists.

**Forum Model**

* Forums operate over Telegram groups.
* Boards and sub‑threads are represented via signed tags and/or in‑message manifests (see §6).
* Manifests & ACL stored as signed fenced blocks in description/pinned message (see §7).
...
* Use `channels.getFullChannel` to read `participants_count`, `about`, `pinnedMsgId`, etc.

**Group‑scoped operations**

* Search within a Board or thread via structured tags: `messages.search` with query `#t:<id>` or `parent:<hash>`.
* Send in a forum: `messages.sendMessage` with composed message body.
* History paging: `messages.getHistory(peer, { addOffset, offsetId, limit })`.

**Moderation**

* Delete: `messages.deleteMessages`.
* Restrict/ban: `channels.editBanned` with appropriate rights.
* Slowmode/locks: `channels.toggleSlowMode`.
...
* **/forum/\:id** — forum overview: Boards list (title, unread, last activity, pinned).
* **/forum/\:id/board/\:boardId** — Board view:
...
* `BoardList`, `SubThreadList`, `MessageList`, `MessageItem`, `Composer`, `PreviewCard`, `MediaViewer`, `BackupPanel`, `ModerationBar`, `AclBadge`.
...
**Problem**: Need intra‑Board sub‑threads discoverable by Telegram’s server search, without metadata collisions.
...
* `BASE32ID` is 4–12 chars, collision‑checked per Board.
...
**ACL Storage**: Signed fenced JSON in description or pinned message:

````
```tgforum-acl v1
{"roles":{"123":"moderator","456":"readonly"},
 "rules":[{"boardId":10,"allow":["admin","moderator"],"deny":["member"]}],
 "sig":"<base64url>"}
```
````

**Enforcement**
- **Telegram‑level**: ban/kick/restrict, delete, slowmode.
- **Client‑level**: hide composer per Board/sub‑thread; auto‑delete violating posts if an admin is online.
...
**Actions**: shadow‑hide locally → if admin, `deleteMessages`; optional `channels.editBanned` for offenders; toggle slowmode.
...
**Scope**: All boards, messages, media, manifests.

**Process**
1. Identify Boards (via manifests or tags).
2. For each Board, page history newest→oldest; store messages + media metadata.
3. Download media on demand or batch (throttled); store as blobs in IndexedDB.
4. Export via JSZip:
   - `/forum.json` (forum & boards metadata)
   - `/boards/<boardId>/thread-<threadId>.json`
   - `/media/<blobId>`
...
## 11) Search & Indexing
- **Server search**: `messages.search` with query `#t:<id>` for sub‑thread recall or `parent:<hash>`.
- **Local index**: Build per‑Board FlexSearch index for message text, authors, and tags; evict LRU boards; cap index size.
- **Jump to last unread**: track per‑Board `lastSeenMsgId` locally.
...
* Login flow to valid session; board list load; send/edit/delete in Board; search `#t:<id>`; media upload.
...
* User can log in, pick a forum, see Boards list, open a Board, see sub‑threads list and messages.
...
3. **Forum load**: `getFullChannel` + boards list; virtualized lists.
4. **Board view**: history paging; message list; composer; media viewer.
5. **Sub‑threads**: tagging rules; parser; board‑scoped search.
...
* Global cross‑board search; move/split threads; sticky rules; board tagging.
